        ...
        **kwargs,
        ...
        if self.transformer_block:
            ...
            filtered_x = torch.gather(
                input=hidden_states, dim=1, index=indices_expanded
            )
            # print(filtered_x.shape)
            import pdb; pdb.set_trace()
            # I think filtered_x goes through the transformer block?
            x_out = self.transformer_block(
                filtered_x,
                attention_mask=torch.gather(attention_mask[0, :,:, 0], 1, indices_expanded[:, :, 0]) if attention_mask is not None else attention_mask,
                position_ids=position_ids[:, :filtered_x.shape[1]],
                past_key_value=past_key_value,
                output_attentions=output_attentions,
                use_cache=use_cache,
                cache_position=cache_position[:filtered_x.shape[1]],
            )[0]
            ...
            out = torch.scatter_add(
                input=hidden_states, dim=1, index=indices_expanded, src=xw_out
            )
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList(
            [MoD(seq_len=1000, dim=config.hidden_size, capacity_factor=0.5, vocab_size=config.vocab_size, transformer_block=LlamaDecoderLayer(config, layer_idx), transformer_depth=8) for layer_idx in range(config.num_hidden_layers)]
        )
